  ...
  return (
    <Stack spacing={2} sx={{ maxWidth: 900 }}>
      ...
      <Paper variant="outlined">
        <List disablePadding>
          {(types.data ?? []).map((type) => (
            <ListItemButton key={type.id} onClick={() => navigate(`/admin/document-types/${type.id}`)} divider>
              <ListItemText
                primary={type.name}
                secondary={<span dir="ltr">{type.code}</span>}
              />
...
function CreateTypeDialog({ onClose, onCreated }: { onClose: () => void; onCreated: (id: string) => void }) {
  ...
  return (
    <Dialog open onClose={busy ? undefined : onClose} fullWidth maxWidth="xs">
      <DialogTitle>{a.newType}</DialogTitle>
      <DialogContent>
