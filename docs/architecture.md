...
| **9 Hardening** **(near close)** | Security headers (+ CSP/COOP), download MIME hardening, `SecuritySuiteTests`, k6 scripts, pen-test checklist (auto-mapped), backup/restore drill — see [`docs/hardening/`](hardening/) | Green security suite ✓; successful drill ✓; staging sign-off for TLS/HSTS/CORS + full k6 against [targets](hardening/performance-targets.md) still open |
